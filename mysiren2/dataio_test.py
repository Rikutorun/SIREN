...
def define_ray(t_n, t_f, N, xp,theta):
    Edge = math.floor(len(xp)/2)
    partitions = np.linspace(t_n, t_f, N, dtype=np.float32)
    centerline_x = partitions*math.sin(math.radians(theta))
    centerline_x = np.expand_dims(centerline_x, axis = 0)
    centerline_y = partitions*-math.cos(math.radians(theta))
    centerline_y = np.expand_dims(centerline_y, axis = 0)
    x_slide = xp/Edge*math.cos(math.radians(theta))
    x_slide = np.expand_dims(x_slide, axis = 1)
    y_slide = xp/Edge*math.sin(math.radians(theta))
    y_slide = np.expand_dims(y_slide, axis = 1)
    
    
    x = centerline_x + x_slide
    x = x.reshape(-1,1)
    x = x.astype(np.float32)
    X = torch.from_numpy(x)
    y = centerline_y + y_slide
    y = y.reshape(-1,1)
...
class InverseHelmholtz(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = square_meshgrid[0, 0, ...].reshape(-1,1)
        y = square_meshgrid[0, 1, ...].reshape(-1,1)
        X = torch.from_numpy(x)
        Y = torch.from_numpy(y)
...
class SingleHelmholtzSource(Dataset):
    def __init__(self, sidelength, velocity='uniform', source_coords=[0., 0.]):
        ...
        fig_ = np.real(self.field).reshape(-1,2)[:,0]
        fig = plt.imshow(np.real(fig_.reshape(-1,128)))
        plt.show()
    ...
    def __getitem__(self, idx):
        ...
        #Y = torch.from_numpy(y)
        #coords = torch.cat((X,Y),dim = 1)
        Dictionary = scipy.io.loadmat('./mysiren2/radon_data/Radon0.mat')
        t_n = -1
        t_f = 1
        N = 128
        xp = Dictionary['xp']
        theta = 0
        coords = define_ray(t_n, t_f, N, xp,theta)
        

        coords_ = torch.zeros(self.sidelength ** 2, 2).uniform_(-1., 1.)
